...
> = {
  ...
  ru: {
    ...
    relatedText: "Связанные страницы помогают руководству сравнить соседние инициативы и выбрать темы, которые стоит разобрать глубже на аудите.",
...
const makeModule = (seed: ModuleSeed): MoldovagazModule => ({
  ...
  layers: seed.layers,
  presentation: {
    valueTitle: seed.presentation?.valueTitle ?? "Что меняется для руководства",
    valueText:
      seed.presentation?.valueText ??
      `${seed.title} превращает разрозненные действия в понятную управленческую область: руководство видит статус, владельца, проблемные зоны и следующий шаг.`,
...
const modules: MoldovagazModule[] = [
  makeModule({
    ...
    outputs: ["единый реестр задач", "статусы и владельцы", "SLA и эскалации", "панель руководства", "картина по регионам"],
    layers: [
      ...
      { title: "Контроль SLA", text: "Руководство видит просрочки, причины задержек и зоны, где процесс требует вмешательства.", icon: "ranking", items: ["SLA", "просрочки", "причины"] },
    ...
    executive: "Если у руководства нет единого экрана по операционным задачам, сложно управлять ответственностью, приоритетами и нагрузкой регионов.",
  ...
  makeModule({
    ...
    outputs: ["модель риска", "список критичных объектов", "профилактические действия", "сигналы для руководства", "история факторов"],
    layers: [
  ...
  makeModule({
    ...
    outputs: ["реестр обязательств", "классификация долгов", "документы и основания", "статусы урегулирования", "отчетность руководству"],
    layers: [
  ...
  makeModule({
    ...
    icon: "monitor",
    summary: "Единый управленческий центр для руководства: операции, регионы, активы, риски, обязательства и ключевые отклонения.",
    ...
    executive: "Панель руководства нужна не для отчетности ради отчетности, а для ежедневного понимания, где компания требует решения.",
...
const groups: MoldovagazGroup[] = [
  {
    ...
    framing: "Сначала Moldovagaz получает видимость того, что происходит в операциях, где есть задержки и какие решения нужны руководству.",
  ...
  },
  {
    ...
    outcomes: ["панель руководства", "аналитика отклонений", "база знаний", "AI-помощник", "контроль качества"],
...
const priorityMatrix: MoldovagazPriorityQuadrant[] = [
  {
    ...
    summary: "Кандидаты, которые могут первыми дать руководству прозрачность, контроль и основу для дальнейших решений.",
...
const project: MoldovagazProject = {
  ...
  client: "Moldovagaz",
  summary: "Презентационная карта из 5 стратегических направлений и 20+ цифровых подпроектов, которые после аудита можно приоритизировать вместе с руководством компании.",
  ...
  icon: "product",
  status: "Концепция для обсуждения с руководством",
  ...
  thesis: {
    title: "Аудит сначала. Решение после фактов.",
    text: "На первом этапе нельзя начинать с заранее выбранного инструмента. Нужно понять реальные процессы Moldovagaz, источники данных, владельцев ответственности и точки, где цифровизация даст управленческий эффект.",
    items: [
  ...
  navigation: {
    ...
    core: "Смысл презентации простой: показать Moldovagaz не одну систему, а карту управляемости компании. Руководство видит, где цифровые решения усиливают операции, инфраструктуру, финансы, внутренние процессы и данные."
  },
  presentationCards: [
    {
      eyebrow: "Слой 1",
    ...
    },
    {
      ...
      text: "После обсуждения становится понятно, какие темы руководство хочет разобрать глубже и какие данные нужно проверить на аудите.",
      items: ["интерес", "ценность", "данные", "ответственные"]
    }
  ],
  implementation: {
    title: "Следующий шаг после встречи",
    text: "После презентации выбираются темы для аудита: процессы, данные, документы, ответственные и текущие инструменты. Это превращает карту развития в понятную основу для дальнейшего проектирования.",
    steps: [
      "Выбрать направления, которые наиболее важны для руководства сейчас.",
  ...
  guardrail: {
    title: "Что должно остаться у руководства",
    text: "После встречи у руководства должна остаться ясная картина: цифровое развитие Moldovagaz можно вести не хаотично, а по понятным направлениям, связанным с реальными процессами компании.",
    items: [
